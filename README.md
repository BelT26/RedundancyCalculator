...
The project was then expanded so that applications could be stored on a Google Sheets API if an employee selected to confirm their application, and viewed, approved or rejected by a member of HR.
...
The figures used are specific to how the company I work at is calculating redundancy in the current tax year.  To allow the programme to be more easily adapted for another company to use at some point in the future, rather than hard code the numbers for tax, holidays etc, I created easily visible global variables which are found at the top of the file. This was the allowances can be changed without a user having to scroll through functions and the tax and NI figures can be easily updated in the next tax year.
...
All 3 python files were passed through the PEP8 validator with no issues.

I entered strings where numbers were required to check that  the is_integer function was validating user input correctly.
...
* The income tax and national insurance contributions were also calculated based on the figures on the UK government website: [income tax bands](https://www.gov.uk/income-tax-rates),  [NI Contributions](https://www.gov.uk/government/publications/rates-and-allowances-national-insurance-contributions/rates-and-allowances-national-insurance-contributions)


## Future Development Possibilities
With more time I would explore the option of adding the following features
* Add the possibility for HR to reverse decisions made after approving or rejecting applications
* Rather than showing each application in chronological order when HR select one of the view options, a summary list of the applicants names and deparments on the chosen sheet could be displayed and HR could decide which one to view.
* At the company I work for there is currently only one person in the HR team.  For larger companies rather than just request a password for HR access I would implement a name and payroll check for the HR member as I did for the employee.  This information would be recorded before the user can access the main HR menu and would be stored and pushed to the API so that it is possible to see who has refused and authorised each application.
* The programme currently relies on the employee entering the correct information when their redundancy is calculated. In a real world scenario the information the user enters for their salary / holidays etc could be automatically validated against an employee database holding these details if they decided to submit an application
